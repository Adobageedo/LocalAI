...
export const DeleteFolder: FC<DeleteFolderProps> = ({
  ...
  const handleDeleteFolderOnly = async () => {
    ...
    // Check if contentType is a valid key in our stateUpdateFunctions object
    if (!(contentType in stateUpdateFunctions)) {
  ...
  const handleDeleteFolderAndItems = async () => {
    ...
    // Only proceed with valid table names for Supabase
    // This ensures we're only using valid table names with Supabase
    ...
    if (!validTableNames.includes(contentType as any)) {
      console.error(`Invalid table name: ${contentType}`)
      return
    }

    const { error } = await supabase
      .from(contentType as any) // Type assertion to handle ContentType
      .delete()
