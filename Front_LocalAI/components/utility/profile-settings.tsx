  ...
  const handleSave = async () => {
    ...
    if (updatedProfile) {
      ...
      const apiKeys = [
        ...
        {
          key: googleGeminiAPIKey,
          provider: "google",
          hasApiKey: !!googleGeminiAPIKey
        },
        {
          key: mistralAPIKey,
          provider: "mistral",
      ...
      apiKeys.forEach(async ({ key, provider, hasApiKey }) => {
        let models: any[] = []
        ...
        if (provider === "openrouter") {
          if (hasApiKey) {
            ...
            setAvailableOpenRouterModels(prev => {
              const newModels = openrouterModels
                ? openrouterModels.filter(
                    (model: OpenRouterLLM) =>
                      !prev.some(
                        prevModel => prevModel.modelId === model.modelId
                      )
                  )
                : []
  ...
  const checkUsernameAvailability = useCallback(
    debounce(async (username: string) => {
      ...
      if (error && error.code === "PGRST116") {
        // PGRST116 means no rows were returned
        // Using type assertion to bypass TypeScript's strict RPC function name checking
        const { data, error: rpcError } = (await (supabase.rpc as any)(
          "is_username_available",
          {
            username
          }
        )) as { data: { is_available: boolean } | null; error: any }
