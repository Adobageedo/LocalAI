...
# Configuration
from backend.core.config import load_config,GMAIL_CLIENT_ID,GMAIL_CLIENT_SECRET,GMAIL_REDIRECT_URI,GMAIL_TOKEN_PATH,OUTLOOK_CLIENT_ID,OUTLOOK_TOKEN_PATH,GMAIL_AUTH_URI,GMAIL_TOKEN_URI, OUTLOOK_SCOPES, ONEDRIVE_SCOPES, OUTLOOK_CALENDAR_SCOPES, GMAIL_SCOPES, GDRIVE_SCOPES, GCALENDAR_SCOPES
...
@router.delete("/microsoft/revoke_access")
...
async def get_google_auth_url(scope: str = None, user=Depends(get_current_user)):
    ...
    logger.debug(f"[GOOGLE OAUTH] Checking credentials for user {user_id} with requested scope {scope}")
    
    # Define scopes based on the requested service
    ...
    if scope == "gmail":
        ...
        service_name = "gcalendar"
    else:
        # Default to all scopes if no specific scope is requested
        selected_scopes = GMAIL_SCOPES + GDRIVE_SCOPES
        service_name = "gmail"
    
    # Check if already authenticated for the requested service
    if creds_status["authenticated"] and creds_status["valid"]:
        # Check if the requested service is already available
        auth_services = check_google_auth_services(user_id)
        
        if service_name in auth_services["services"]:
            logger.debug(f"[GOOGLE OAUTH] Valid token found for {service_name}. Already authenticated.")
            return JSONResponse({"authenticated": True})
    
    # Otherwise, generate the OAuth URL with the appropriate scopes
    logger.debug(f"[GOOGLE OAUTH] Generating auth URL with scopes: {selected_scopes}")
...
async def google_oauth2_callback(request: Request):
    ...
    if state_encoded:
        try:
            ...
            user_id = state_json.get("user_id", user_id)
            requested_scope = state_json.get("scope")
            logger.debug(f"[GOOGLE OAUTH] Decoded state: user_id={user_id}, scope={requested_scope}")
        except Exception as e:
    ...
    logger.debug(f"[GOOGLE OAUTH] User ID: {user_id}, Requested scope: {requested_scope}")
