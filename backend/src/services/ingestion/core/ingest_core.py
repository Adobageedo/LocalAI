...
def batch_ingest_documents(batch_documents, user, collection=None, file_registry=None):
    ...
    if split_docs:
        try:
            ...
            # Use smaller batch size for large ingestions to avoid timeouts
            qdrant_batch_size = 10 if len(split_docs) > 1000 else 20
            logger.info(f"Using Qdrant batch size: {qdrant_batch_size}")
            manager.add_documents_in_batches(split_docs, batch_size=qdrant_batch_size)
