...
logger = log.bind(name="src.services.db.test_postgres_connection")
logger.info(f"Loaded environment from: {os.path.abspath('.env') if os.path.exists('.env') else 'No .env file found'}")

from src.core.constants import POSTGRES_DB, POSTGRES_USER, POSTGRES_PASSWORD, POSTGRES_HOST, POSTGRES_PORT
