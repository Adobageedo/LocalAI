...
def set_imap_status(user, status):
    ...
    logger.info(f"IMAP STATUS: User {user} status set to {status}")

def get_imap_status(user):
    status = imap_ingest_status.get(user, "not_started")
    logger.info(f"IMAP STATUS: Retrieved status for user {user}: {status}")
    return status
...
def ingest_imap_emails(creds: ImapCredentials,user=Depends(get_current_user)):
    ...
    # Déterminer l'utilisateur pour le statut et les métadonnées
    user = creds.imap_user or creds.user
    logger.info(f"IMAP INGEST: Starting ingestion for user {user}")
