...
class ImapEmailSource(EmailSource):
    ...
    def fetch_emails(self, **kwargs) -> Tuple[EmailFetchResult, List[Email]]:
        ...
        try:
            ...
            # Essayons d'abord avec les dossiers par défaut pour ce serveur spécifique
            # Pour newsflix.fr, on sait que les dossiers sont probablement INBOX et INBOX.Sent
            default_folders = ['INBOX', 'INBOX.Sent']
            available_folders = default_folders
            
            # Essayons aussi de lister les dossiers disponibles
            try:
                status, folder_list = mail.list()
                raw_folders = []
                
                if status == 'OK':
                    self.logger.debug(f"Raw folder list: {[f.decode() for f in folder_list]}")
                    
                    for folder_info in folder_list:
                        folder_info_str = folder_info.decode()
                        self.logger.debug(f"Processing folder info: {folder_info_str}")
                        
                        # Extraire le nom du dossier en fonction du format
                        if '\"' in folder_info_str:
                            # Format avec guillemets doubles échappés
                            parts = folder_info_str.split('\"')
                            if len(parts) >= 2:
                                folder_name = parts[1]
                                raw_folders.append(folder_name)
                        elif '"' in folder_info_str:
                            ...
                            # Essayer d'extraire le dernier élément
                            parts = folder_info_str.split(' ')
                            if len(parts) >= 1:
                                folder_name = parts[-1]
                                raw_folders.append(folder_name)
                    
                    # Si on a trouvé des dossiers, on les ajoute à notre liste
                    if raw_folders:
                        available_folders.extend(raw_folders)
                        
                    # Ajouter aussi des variations courantes
                    available_folders.extend(['Sent', 'Sent Items', 'Sent Mail', 'Envoyés'])
                    
                    # Dédupliquer la liste
                    available_folders = list(set(available_folders))
                    
                    self.logger.info(f"Available folders: {', '.join(available_folders)}")
            except Exception as e:
                self.logger.warning(f"Error listing folders: {str(e)}")
                # En cas d'erreur, on utilise les dossiers par défaut
            ...
            # Only call close() if we've selected a folder
            try:
                mail.close()
            except Exception as close_err:
                # Ignore close errors, as we might not have selected a folder
                pass
