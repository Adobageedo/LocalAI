...
import llmClient, { ChatMessage } from '../api/utils/llmClient';
...
router.post('/', async (req: Request, res: Response) => {
  try {
        ...
        const ragResponse = await fetch(
          `${process.env.RAG_API_URL || 'https://easier-snappily-ansley.ngrok-free.dev/api/rag/search'}`,
          {
            ...
            headers: { 
              ...
              'x-api-key': process.env.RAG_API_KEY || 'W1eqZEROOsKw9gphfEYPvPYlHqS0lSAELjbYJCWqCxFl831wqSmwlXTht6t4ABO0'
            },
            body: JSON.stringify({
    ...
    let mcpTools = null;
    if (useMcpTools) {
      const { getMcpTools } = await import('../api/utils/mcp');
    ...
    if (mcpTools && mcpTools.length > 0) {
      ...
      if (firstResponse.tool_calls && firstResponse.tool_calls.length > 0) {
        ...
        const { executeMcpTool } = await import('../api/utils/mcp');
