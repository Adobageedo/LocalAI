...
import { LLM_QUICK_ACTIONS_DICTIONARY, getLLMQuickAction } from '../../../config/llmQuickActions';
...
interface TemplateChatInterfaceProps {
  ...
  tone?: string;  // Optional tone preference
...
const TemplateChatInterface: React.FC<TemplateChatInterfaceProps> = ({
  ...
  // Generate conversationId from email or use fallback
  const conversationId = currentEmail?.conversationId || currentEmail?.internetMessageId || 'default-conversation';

  // Build emailContext from OfficeContext
  const emailContext = {
  ...
  // Check if email has attachments (directly from Office.js)
  ...
  useEffect(() => {
    if (quickActionContext.state.isActive && quickActionContext.state.streamedContent) {
      setMessages(prev => {
        // Get the action config to get the user prompt
        const actionKey = quickActionContext.state.actionKey;
        ...
        const hasQuickActionUserMessage = prev.some(m => m.id === userMessageId);
        const hasQuickActionAssistantMessage = prev.some(m => m.id === assistantMessageId);
        
        const newMessages = [...prev];
        
        // Add user message first (only once per action)
        if (!hasQuickActionUserMessage && actionConfig) {
          newMessages.push({
            id: userMessageId,
            role: 'user' as const,
            content: actionConfig.userPrompt,
        ...
        if (hasQuickActionAssistantMessage) {
          // Update existing assistant message for this action
  ...
  }, [quickActionContext.state.streamedContent, quickActionContext.state.isActive, quickActionContext.state.actionKey]);
