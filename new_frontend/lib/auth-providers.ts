...
export const authProviders = {
  ...
  getUserCapabilities: async (): Promise<{
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`Failed to get user capabilities: ${response.statusText}`);
      }
   ...
   */
  checkAuthStatus: async (provider: string): Promise<{ authenticated: boolean; email?: string }> => {
    try {
      // Determine the correct API endpoint based on provider
      let endpoint = '';
      
      if (provider === 'gmail' || provider === 'gdrive' || provider === 'gcalendar') {
        endpoint = `${process.env.NEXT_PUBLIC_BACKEND_URL || 'http://localhost:8000'}/api/db/gdrive/auth_status`;
      } else if (provider === 'outlook' || provider === 'outlook_calendar' || provider === 'onedrive') {
   ...
   */
  startIngestion: async (provider: string, options: { forceReingest?: boolean } = {}): Promise<any> => {
    try {
      ...
      const response = await clientAuthFetch(`${process.env.NEXT_PUBLIC_BACKEND_URL || 'http://localhost:8000'}/api/sources/sync/start`, {
        method: 'POST',
