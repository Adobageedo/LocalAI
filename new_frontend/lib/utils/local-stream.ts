...
export async function LocalAIStream(
  ...
  console.log(`[${new Date().toISOString()}] LocalAIStream function called with response status:`, response.status);
  ...
  (async () => {
    try {
      ...
      console.log(`[${new Date().toISOString()}] Response content type:`, contentType, 'Is SSE:', isSSE);
      ...
      if (isSSE) {
          ...
          for (const line of lines) {
            ...
            if (line.startsWith('data: ')) {
              ...
              try {
                ...
                if (data.hasOwnProperty('chunk') || data.hasOwnProperty('text')) {
                  const content = data.chunk !== undefined ? data.chunk : (data.text !== undefined ? data.text : '');
                  fullText += content;
        ...
        if (sources.length > 0) {
          const sourcesText = '\n\n**Sources:**\n' + 
            sources.map((source: any, index: number) => {
              const filename = source.filename || source.name || `Source ${index + 1}`;
        ...
        if (responseData.sources && responseData.sources.length > 0) {
          ...
          responseData.sources.forEach((source: any, index: number) => {
            ...
            const url = source.url || '';
            // If URL is available, format as markdown link
            formattedAnswer += url ? `- [${filename}](${url})\n` : `- ${filename}\n`;
          });
        }
