...
export const SyncStatusBar = () => {
  ...
  const getStatusIcon = (syncStatus: SyncStatus) => {
    ...
    switch (status) {
      ...
      default:
        return <RefreshCw className="mr-2 h-4 w-4 text-gray-500" />
    }
  }
  ...
  const fetchSyncStatuses = async () => {
    try {
      ...
      if (syncIds.length === 0) {
        try {
          const response = await clientAuthFetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/sync/status`)
          
          // If the endpoint returns 405 Method Not Allowed, the endpoint doesn't exist
          if (response.status === 405) {
            console.log('Sync status endpoint not available, skipping general status check')
            return
          }
          ...
          if (response.ok) {
            ...
            if (data && Array.isArray(data)) {
              const activeStatuses = data.filter(
                (status: SyncStatus) => status.status === 'pending' || status.status === 'in_progress'
              )
              ...
              if (activeStatuses.length > 0) {
                ...
                const newSyncIds = activeStatuses.map(status => 
                  `${status.id}|${Date.now()}`
                )
                localStorage.setItem('activeSyncIds', JSON.stringify(newSyncIds))
                return
              }
            }
          }
        } catch (error) {
          console.error('Error fetching sync statuses from main endpoint:', error)
        }
      }
      ...
      if (syncIds.length > 0) {
        ...
        for (const idWithTimestamp of syncIds) {
          ...
          try {
            const response = await clientAuthFetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/sync/status/${syncId}`)
            if (response.ok) {
              const data = await response.json()
  ...
  return (
    <div className="fixed top-0 left-0 right-0 z-50 animate-in fade-in slide-in-from-top-5">
      <div className="bg-background/95 backdrop-blur-sm border-b border-border shadow-sm py-2 px-4 flex items-center justify-between">
        <div className="flex flex-col flex-grow mr-2">
          <div className="flex items-center mb-1">
            {syncStatuses[0] && getStatusIcon(syncStatuses[0])}
            <p className="text-sm font-medium truncate max-w-[80vw]">
