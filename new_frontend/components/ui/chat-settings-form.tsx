...
export const ChatSettingsForm: FC<ChatSettingsFormProps> = ({
  ...
  // Always use GPT-4.1 Mini
  useEffect(() => {
    if (chatSettings.model !== "gpt-4.1-mini") {
      onChangeChatSettings({ ...chatSettings, model: "gpt-4.1-mini" })
    }
  }, [chatSettings.model])
  ...
  return (
    <div className="space-y-3">
      <div className="space-y-1">
        <Label>Model</Label>
        <div className="flex items-center space-x-2 rounded-md border-2 p-2">
          <div className="text-sm font-medium">GPT-4.1 Mini</div>
        </div>
      </div>

      <div className="space-y-1">
        <div className="flex items-center justify-between">
          <Label>Use Personal Document Retrieval</Label>
          <Switch
            checked={chatSettings.useRetrieval || false}
            onCheckedChange={(checked) =>
              onChangeChatSettings({ ...chatSettings, useRetrieval: checked })
            }
          />
        </div>
        <div className="text-muted-foreground text-xs">
          When enabled, the AI will search through your personal documents to provide more relevant answers.
        </div>
      </div>
