...
import { IconBrandGoogle, IconMail, IconCalendar, IconCloudUpload, IconCheck, IconX, IconBrandWindows, IconRefresh, IconDownload } from "@tabler/icons-react"
...
export const Connections: FC<ConnectionsProps> = ({}) => {
  ...
  const handleConnect = async (service: string) => {
    ...
    try {
      ...
      toast.error(`Failed to connect to ${service}. ${error instanceof Error ? error.message : "Please try again."}`)
    } finally {
      setLoading(null)
    }
  }
  ...
  const handleDisconnect = async (service: string) => {
    ...
    try {
      ...
      toast.error(`Failed to disconnect from ${service}. ${error instanceof Error ? error.message : "Please try again."}`)
    } finally {
      setLoading(null)
    }
  }
  ...
  const handleStartSync = async (service: string) => {
    ...
    try {
      ...
      toast.error(`Failed to start synchronization for ${service}. ${error instanceof Error ? error.message : "Please try again."}`)
    } finally {
  ...
  const renderConnectionCard = (
    ...
    return (
      <Card className="mb-4">
        ...
        <CardContent>
          {isConnected && email && (
            <p className="text-muted-foreground mb-2 text-sm">Connected as: {email}</p>
          )}
        </CardContent>
  ...
  const isGoogleConnected = connectionStatus.gmail || connectionStatus.gdrive || connectionStatus.gcalendar
  const isMicrosoftConnected = connectionStatus.outlook || connectionStatus.onedrive || connectionStatus.outlook_calendar
  ...
  return (
    <div className="h-full overflow-auto p-4">
      ...
      <p className="text-muted-foreground mb-6 text-sm">
        Connect to external services to access your emails, calendar events, and files.
      </p>

      <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as "google" | "microsoft")}>
