...
export async function POST(request: Request) {
  ...
  try {
    ...
    const payload: {
      ...
      temperature: typeof chatSettings.temperature === 'number' ? chatSettings.temperature : 0.7,
    ...
    if (typeof chatSettings.includeProfileContext !== 'undefined') {
      payload.include_profile_context = Boolean(chatSettings.includeProfileContext);
    }
    ...
    const response = await authFetch('https://chardouin.fr/api/prompt', {
        method: 'POST',
    ...
    console.log('Response from backend received, status:', response.status, 
